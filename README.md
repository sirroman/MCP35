...
| **сборка .cfe** | Соберите расширение из `Module.bsl` (**v6.3.0**) — см. `HOW_TO_BUILD_CFE.md` |
...
- **Безопасность на стороне 1С** (6.3.0): политика доступа к инструментам (allow/deny) и авторизация пользователей MCP — настройки в начале `Module.bsl` (`РазрешённыеПользователиMCP` / `ЗапрещённыеИнструментыMCP`). По умолчанию опасные инструменты запрещены, доступ ограничен `mcp_agent`. См. `SECURITY.md`.
- **Исправлен `get_changes_since`** (6.3.0): корректные идентификаторы событий журнала (`_$Data$_.*`), сортировка по времени, инклюзивный `watermark` (доказывает отсутствие изменений), поле `truncated`.
- **Восстановление привилегированного режима** в `get_rights_for_user` (6.3.0) даже при исключении.
...
```json
{
  ...
  "result": {
    ...
    "capabilities": { "tools": {} },
    "serverInfo": { "name": "infaton-1c-mcp", "version": "6.3.0" }
  }
}
```
