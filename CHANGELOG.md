...
### Исправлено
- **Тихий «успех» при несохранённой записи.** `create_object`, `update_object` и
  `update_tabular_section` больше не глушат ошибки установки реквизитов пустым `Попытка…Исключение`.
  Ошибки собираются в поле ответа `attribute_errors` (`attribute`, `error`, `context` вида
  `Товары[1]`), и при их наличии `success:false`. Общий помощник `ДобавитьОшибкуРеквизита`.
- **Не создавался договор контрагента с владельцем.** `Владелец` (как и `Дата`, `Номер`, `Родитель`)
  — стандартный реквизит, которого нет в `Метаданные().Реквизиты`. Новый `МСП_ТипРеквизита` ищет тип
  и в `СтандартныеРеквизиты`, поэтому `Владелец_guid` теперь резолвится и сохраняется.
- **Даты из JSON не записывались.** Строка-дата присваивалась реквизиту как есть (`ДатаРождения`
  терялась, `Дата` документа оставалась пустой → ошибка записи). Добавлена конвертация строки в `Дата`
  через `ПреобразоватьДату` (ISO 8601 / `YYYYMMDD` / `DD.MM.YYYY`) для реквизитов датного типа.

### Изменено
- **Атомарность обновления.** `update_object` и `update_tabular_section` при любой ошибке установки
  реквизита **не вызывают `Записать()`** — существующие данные остаются нетронутыми (частичная запись
  при `success:false` исключена). `create_object` намеренно оставлен частичным: возвращает `guid` и
  сохранённые скаляры вместе с `attribute_errors`.
- **Ссылка строкой-UUID без суффикса `_guid`.** Значение реквизита ссылочного типа теперь резолвится
  и без суффикса (например, `Владелец = "UUID"`): либо корректная ссылка, либо явная ошибка.
- **`МСП_РазрешитьСсылку`** переведён на приём `ОписаниеТипов` вместо метаданных реквизита (нужно для
  стандартных реквизитов).

### Проверено
- Полный регресс-прогон по `INFATON_MCP_REGRESSION_TEST_CASES.md` на тестовой базе
  (БП 3.0.168.19, платформа 8.3.24.1691) — **PASS**, активных тестовых объектов после уборки `0`.
